<?php
  ...
  $upload_dir = '../files/newsletters/';
  if($action == 1) {
    $id=$_POST['id'];
    $date=mysql_real_escape_string($_POST['date']);
    $title=mysql_real_escape_string($_POST['title']);
    $msg = upload_file_to_dir($_FILES,$upload_dir,'doc',NEWSLETTERS_FILE_SIZE);
    if($msg['value'] == 1) {
      $old_file = mysql_query("select file from newsletters where id = '$id'");
	...
	$message = 'An error occured while updating the newsletter. Error code:SKRSS11.';
	...
	$is_message_error = true;
      ...
      if(mysql_query("update newsletters set title='$title', date='$date' where id=$id"))
	$message .= 'The newsletters was updated successfully.';
      else {
	$message .= 'An error occured while updating the newsletter. Error code:SKRSS10.';
	$is_message_error = true;
      }
    }
  }
  if($action == 2) {
    ...
    $assoc = mysql_fetch_assoc($old_file);
    $old_file = $assoc['file'];
    if(mysql_query("delete from newsletters where id=$id")) {
      $message = 'The newsletter was deleted successfully.';
      if($old_file != '' && file_exists($upload_dir.$old_file))
	unlink($upload_dir.$old_file);
    }
    else {
      $message = 'An error occured while deleting the newsletter. Error code:SKRSS7.';
      $is_message_error = true;
    }
  }
  if($action == 3) {
    ...
    if(mysql_query("insert into newsletters(title,date,file) values('$title','$date','$file')"))
      $message .= 'The newsletter was added successfully.';
    else {
      $message .= 'An error occured while creating the new newsletter. Error code:SKRSS8.';
      $is_message_error = true;
    }
  }
  $newsletters = mysql_query("select * from newsletters");
...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <title>SS Admin | Newsletter</title>
    <link href="loginmodule.css" rel="stylesheet" type="text/css" />
    <script type="text/javascript">
      function confirmDelete(id){
        var to_delete = window.confirm("Are you sure you want to delete this newsletter?");
        if(to_delete) {
  ...
  <body>
    ...
    <?php if(mysql_num_rows($newsletters)>0) {?>
