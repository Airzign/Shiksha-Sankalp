<?php
  ...
  $upload_dir = '../files/tax_returns/';
  if($action == 1) {
    $id=$_POST['id'];
    $organization=mysql_real_escape_string($_POST['organization']);
    $title=mysql_real_escape_string($_POST['title']);
    $msg = upload_file_to_dir($_FILES,$upload_dir,'doc',TAX_RETURNS_FILE_SIZE);
    if($msg['value'] == 1) {
      $old_file = mysql_query("select file from tax_returns where id = '$id'");
      $assoc = mysql_fetch_assoc($old_file);
      $old_file = $assoc['file'];
      $file = $msg['msg'];
      if(mysql_query("update tax_returns set title='$title', organization='$organization', file='$file' where id=$id")) {
	$message = 'The tax return was updated successfully.';
	if($old_file != '' && $old_file != $file && file_exists($upload_dir.$old_file))
	  unlink($upload_dir.$old_file);
      }
      else {
	$message = 'An error occured while updating the tax return. Error code:SKRSS11.';
	...
	$is_message_error = true;
      ...
      if(mysql_query("update tax_returns set title='$title', organization='$organization' where id=$id"))
	$message .= 'The tax_returns was updated successfully.';
      else {
	$message .= 'An error occured while updating the tax return. Error code:SKRSS10.';
	$is_message_error = true;
      }
    }
  }
  if($action == 2) {
    ...
    $assoc = mysql_fetch_assoc($old_file);
    $old_file = $assoc['file'];
    if(mysql_query("delete from tax_returns where id=$id")) {
      $message = 'The tax return was deleted successfully.';
      if($old_file != '' && file_exists($upload_dir.$old_file))
	unlink($upload_dir.$old_file);
    }
    else {
      $message = 'An error occured while deleting the tax return. Error code:SKRSS7.';
      $is_message_error = true;
    }
  }
  if($action == 3) {
    ...
    if(mysql_query("insert into tax_returns(title,organization,file) values('$title','$organization','$file')"))
      $message .= 'The tax return was added successfully.';
    else {
      $message .= 'An error occured while creating the new tax return. Error code:SKRSS8.';
      $is_message_error = true;
    }
  }
  $tax_returns = mysql_query("select * from tax_returns");
  if($tax_returns == false) {
...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <title>SS Admin | Tax Return</title>
    <link href="loginmodule.css" rel="stylesheet" type="text/css" />
    <script type="text/javascript">
      function confirmDelete(id){
        var to_delete = window.confirm("Are you sure you want to delete this tax return?");
        if(to_delete) {
  ...
  <body>
    ...
    <?php if(mysql_num_rows($tax_returns)>0) {?>
      ...
      <table>
        ...
        <?php while($row=mysql_fetch_assoc($tax_returns)) { ?>
        <tr>
