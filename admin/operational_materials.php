<?php
  ...
  if($action == 1) {
    ...
    $msg = upload_file_to_dir($_FILES,$upload_dir,'doc',OPERATIONAL_MATERIALS_FILE_SIZE);
    if($msg['value'] == 1) {
      $old_file = mysql_query("select file from operational_materials where id = '$id'");
	...
	$message = 'An error occured while updating the operational material. Error code:SKRSS11.';
	...
	$is_message_error = true;
      ...
      if(mysql_query("update operational_materials set title='$title', date='$date' where id=$id"))
	$message .= 'The operational_materials was updated successfully.';
      else {
	$message .= 'An error occured while updating the operational material. Error code:SKRSS10.';
	$is_message_error = true;
      }
    }
  }
  if($action == 2) {
    ...
    $assoc = mysql_fetch_assoc($old_file);
    $old_file = $assoc['file'];
    if(mysql_query("delete from operational_materials where id=$id")) {
      $message = 'The operational material was deleted successfully.';
      if($old_file != '' && file_exists($upload_dir.$old_file))
	unlink($upload_dir.$old_file);
    }
    else {
      $message = 'An error occured while deleting the operational material. Error code:SKRSS7.';
      $is_message_error = true;
    }
  }
  if($action == 3) {
    ...
    if(mysql_query("insert into operational_materials(title,date,file) values('$title','$date','$file')"))
      $message .= 'The operational material was added successfully.';
    else {
      $message .= 'An error occured while creating the new operational material. Error code:SKRSS8.';
      $is_message_error = true;
    }
  }
  $operational_materials = mysql_query("select * from operational_materials");
  if($operational_materials == false) {
...
<html xmlns="http://www.w3.org/1999/xhtml">
  <head>
    ...
    <title>SS Admin | Operational Materials</title>
    <link href="loginmodule.css" rel="stylesheet" type="text/css" />
    <script type="text/javascript">
      function confirmDelete(id){
        var to_delete = window.confirm("Are you sure you want to delete this operational material?");
        if(to_delete) {
  ...
  <body>
    <h1>Operational Material Admin </h1>
    ...
    <?php if(mysql_num_rows($operational_materials)>0) {?>
      ...
      <table>
        ...
        <?php while($row=mysql_fetch_assoc($operational_materials)) { ?>
        <tr>
